...
The triangle simulates 2D Motion. Both direction and orientation are momentum-based, meaning the character will accelerate and decelerate to the respective direction interpreted by the joystick. The position and orientation of the character can be translated into CV and MIDI CC to be used as a controller for musical devices ranging from Eurorack to VST plugins. 

This project is currently unfinished, however future plans with this project involve refactoring the procedurally-oriented C++ code to an object-oriented system, porting the development environment to PlatformIO, and fleshing out gameplay mechanics to better resemble a top-down spaceship shooter like the retro game Asteroids.
